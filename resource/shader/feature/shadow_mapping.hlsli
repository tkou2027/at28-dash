    ...
    float4 pos_view_light, float3 normal, float3 lightDir)
{
    ...
    bool outside = dir_light_pos.x <= 0.01 || dir_light_pos.x >= 0.99 ||
                   dir_light_pos.y <= 0.01 || dir_light_pos.y >= 0.99 ||
                   dir_light_pos.z <= 0.01 || dir_light_pos.z >= 0.99;
    float facing = step(0.0f, dot(normal, -lightDir));
    ...
    percentLit /= 9.0f;
    percentLit = saturate(percentLit + outside + (1.0 - facing));
    return percentLit;
}
