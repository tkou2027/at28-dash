...
DirectX::XMMATRIX CameraMath::CalculateProjectionMatrix(const CameraShapeConfig& config)
{
	...
	{
		const float tan_half_fov = config.fov * 0.5f;
...
DirectX::XMMATRIX CameraMath::CalculateObliqueProjection(const DirectX::XMMATRIX& proj_matrix, const Vector4& plane_v)
{
	...
	XMVECTOR q_clip_vector = q_clip.ToXMVECTOR();
	//XMMATRIX inv_proj = DirectX::XMMatrixInverse(nullptr, proj_matrix);
	//XMVECTOR q_view_vector = DirectX::XMVector4Transform(q_clip_vector, inv_proj);

	Vector4 q_view{
		q_clip.x / proj_matrix.r[0].m128_f32[0],
		q_clip.y / proj_matrix.r[1].m128_f32[1],
		1.0f / proj_matrix.r[3].m128_f32[2], // 1.0f,
		(
			1.0f / proj_matrix.r[2].m128_f32[3]
				- (proj_matrix.r[2].m128_f32[2] / (proj_matrix.r[2].m128_f32[3] * proj_matrix.r[3].m128_f32[2]))
		)
	};
