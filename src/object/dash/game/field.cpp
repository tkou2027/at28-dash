...
void Field::Initialize()
{
	...
	m_fade = fade;
	auto dash_controller = GetOwner().CreateGameObject<DashController>();
	dash_controller->SetField(std::dynamic_pointer_cast<Field>(shared_from_this()));
	m_dash_controller = dash_controller;
	auto time_controller = GetOwner().CreateGameObject<TimeController>();
...
DashController& Field::GetDashController() const
{
	return *m_dash_controller.lock();
}

TimeController& Field::GetTimeController() const
{
	return *m_time_controller.lock();
}