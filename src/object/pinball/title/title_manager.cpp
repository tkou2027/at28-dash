...
void TitleManager::Initialize()
{
	...
	// point light
	//auto point_light = GetOwner().CreateGameObject<PointLight>();
	//point_light->SetColor(Vector3{ 1.0f, 1.0f, 1.0f } *5.0f);
	////point_light->GetTransform().SetPosition(Vector3{ -3.0f, 5.0f, -1.0f } *20.0f);
	//point_light->SetLightActive(true);
	// objects end ========
	...
	screen->GetTransform().SetRotationYOnly(Math::PI);
