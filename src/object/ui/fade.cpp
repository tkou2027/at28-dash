...
void Fade::Initialize()
{
	...
	for (int i = 0; i < NUM_SPRITES; i++)
	{
		...
		sprite.m_uv_animation_state.uv_scroll_size.x = SCREEN_WIDTH / 10.0f;
...
{
	m_countdown.Initialize(m_config.duration);
	...
	render_comp.GetSprite(0).m_transform.SetPositionY(0.0f);
	render_comp.GetSprite(1).m_transform.SetPositionY(0.0f);
...
void Fade::StartFadeOut(bool fade_out_in)
{
	...
	auto& render_comp = m_components.Get<ComponentRendererSprite>(m_comp_id_render);
	render_comp.GetSprite(0).m_transform.SetPositionY(-SCREEN_HEIGHT);
	render_comp.GetSprite(1).m_transform.SetPositionY(SCREEN_HEIGHT);
...
{
	float t = m_countdown.Update(GetDeltaTime());
	if (t <= 0.0f)
	{
		if (m_fade_in_out)
		{
	...
	auto& render_comp = m_components.Get<ComponentRendererSprite>(m_comp_id_render);
	render_comp.GetSprite(0).m_transform.SetPositionY(-SCREEN_HEIGHT * t);
	render_comp.GetSprite(1).m_transform.SetPositionY(SCREEN_HEIGHT * t);
}
