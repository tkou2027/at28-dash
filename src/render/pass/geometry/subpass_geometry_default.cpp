#include "subpass_geometry_default.h"
...
void SubPassGeometryDefault::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	PassBaseGeometry::Initialize(device, context);
	m_pixel_shader = Shader::CreateShaderPixel(m_device, L"pixel_geometry.cso");

	// constant buffer per material
	m_cb_material = Buffer::CreateConstantBuffer(m_device, sizeof(MaterialDefault));
}

bool SubPassGeometryDefault::ShouldRender(const MaterialDesc& material_desc, ModelType model_type) const
{
...
void SubPassGeometryDefault::SetInfoPerDraw()
{
...
void SubPassGeometryDefault::SetInfoPerModel(const ModelRenderInfo& model)
{
	PassBaseGeometry::SetInfoPerModel(model);
	SetInfoPerMaterial(model.key);
}

void SubPassGeometryDefault::SetInfoPerMaterial(const ModelRenderKey& model_key)
{
	// per object material
