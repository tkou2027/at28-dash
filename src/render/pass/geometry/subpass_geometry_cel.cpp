#include "subpass_geometry_cel.h"
...
namespace
{
	struct CelMaterial
	{
		DirectX::XMFLOAT3 g_light_dir_w;
		float g_ramp_offset;
	};
}

void PassGeometryCel::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	PassBaseGeometry::Initialize(device, context);
	m_pixel_shader = Shader::CreateShaderPixel(m_device, L"pixel_geometry_cel.cso");
	m_cb_material = Buffer::CreateConstantBuffer(m_device, sizeof(MaterialDefault));
	m_cb_material_cel = Buffer::CreateConstantBuffer(m_device, sizeof(CelMaterial));
}

bool PassGeometryCel::ShouldRender(const MaterialDesc& material_desc, ModelType model_type) const
{
	if (!material_desc.IfTechnique(RenderTechnique::DEFAULT, m_curr_render_layer)
		|| !material_desc.IfTechnique(RenderTechnique::DEFERRED_CEL, m_curr_render_layer))
	{
		return false;
	}
...
void PassGeometryCel::SetInfoPerDraw()
{
	...
	m_context->PSSetSamplers(0, 2, render_states.m_ss_linear_clamp.GetAddressOf());
	// ps cb
	...
	m_context->PSSetConstantBuffers(2, 1, m_cb_material.GetAddressOf());
	m_context->PSSetConstantBuffers(3, 1, m_cb_material_cel.GetAddressOf()); // cel config
	// om
...
void PassGeometryCel::SetInfoPerModel(const ModelRenderInfo& model)
{
	// per object
...
void PassGeometryCel::SetInfoPerMaterial(const ModelRenderKey& model_key)
{
	// per object material
	...
	// TODO: ramp map texture
	{
		auto& texture_loader = GetTextureLoader();
		const auto texture_id = texture_loader.GetOrLoadTextureFromFile("asset/texture/test_ramp.png", DXGI_FORMAT_R8G8B8A8_UNORM_SRGB);
		const auto texture_view = texture_loader.GetTexture(texture_id);
		m_context->PSSetShaderResources(5, 1, texture_view.GetAddressOf());
	}
	...
	{
		//const auto& material_cel = material_desc.GetTechnique<TechniqueDescDeferredCel>(m_curr_render_layer);
		CelMaterial cb{};
		cb.g_light_dir_w = g_shader_setting.light_direction.ToXMFLOAT3(); // TODO
		cb.g_ramp_offset = g_shader_setting.cel_offset / 6.0f; // TODO
		m_context->UpdateSubresource(m_cb_material_cel.Get(), 0, nullptr, &cb, 0, 0);
	}

	if (material.stencil_ref >= 0)
	{
		m_context->OMSetDepthStencilState(
			render_states.m_dss_depth_write_leq_stencil_read_eq.Get(), material.stencil_ref);
	}
	else
	{
		m_context->OMSetDepthStencilState(render_states.m_dss_depth_enabled.Get(), 0);
