...
#include "render/pass/forward/subpass_forward_shadow_stylized.h"
...
void RenderPathMain::Initialize()
{
	...
	m_pass_forward.AddSubPass(std::make_unique<SubPassForwardMask>(), PassForward::SubPassQueue::PRE);
	m_pass_forward.AddSubPass(std::make_unique<SubPassForwardUnlit>(), PassForward::SubPassQueue::DEFAULT);
	m_pass_forward.AddSubPass(std::make_unique<SubPassForwardShadowStylized>(), PassForward::SubPassQueue::DEFAULT);
...
void RenderPathMain::Draw(RenderViewKey view_key, const ViewContext& view_context)
{
	...
	m_pass_depth.Draw();
	m_pass_forward.SetDrawingQueue(PassForward::SubPassQueue::PRE);
	m_pass_forward.Draw(); // mask
	m_render_targets.pre_pass.ClearDepthStencil(context, D3D11_CLEAR_DEPTH);
	...
	if (m_gpu_timers[1].TryGetTime(nullptr))
	{
		...
		hal::dout << "GPU time: skybox " << avg_time * 1000.0f << " ms" << std::endl;
	}
