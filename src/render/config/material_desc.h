...
struct TechniqueDescForwardMask
{
	static constexpr RenderTechnique TYPE{ RenderTechnique::FORWARD_MASK };
	int stencil_ref{};
};
...
struct TechniqueDescForwardShadowStylized
{
	static constexpr RenderTechnique TYPE{ RenderTechnique::FORWARD_SHADOW_STYLIZED };
	TextureResourceId shadow_texture_id{};
	TextureResourceId world_texture_id{};
	DirectX::XMFLOAT4X4 light_mat_view_proj{}; // dirlight
...
public:
	using Variant = std::variant<
		...
		TechniqueDescForwardMask,
		TechniqueDescForwardUnlit,
		TechniqueDescForwardSilhouette,
		TechniqueDescForwardShadowStylized,
	...
	TechniqueDesc(const TechniqueDescForwardMask& desc) : m_data(desc), layer_mask(CameraRenderLayerMask::DEFAULT) {}
	TechniqueDesc(const TechniqueDescForwardUnlit& desc) : m_data(desc), layer_mask(CameraRenderLayerMask::DEFAULT) {}
	TechniqueDesc(const TechniqueDescForwardShadowStylized& desc) : m_data(desc), layer_mask(CameraRenderLayerMask::DEFAULT) {}
