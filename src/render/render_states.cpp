...
void RenderStates::InitializeStates()
{
	...
	}
	{
		...
		m_device->CreateDepthStencilState(&dsd, m_dss_depth_read_leq_stencil_read_eq.GetAddressOf());
	}
	{
		D3D11_DEPTH_STENCIL_DESC dsd = {};
		dsd.DepthEnable = TRUE;
		dsd.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL; // enable write
		dsd.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
		// enable stencil
		dsd.StencilEnable = TRUE;
		dsd.StencilReadMask = 0xFF;
		...
		// back face
		dsd.BackFace = dsd.FrontFace;
		m_device->CreateDepthStencilState(&dsd, m_dss_depth_write_leq_stencil_read_eq.GetAddressOf());
	}
	{
		// enable depth read(less equal), no write 
		D3D11_DEPTH_STENCIL_DESC dsd = {};
