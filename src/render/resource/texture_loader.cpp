...
ComPtr<ID3D11ShaderResourceView> TextureLoader::GetOrLoadTextureFromMemory(const std::string& filename, const aiTexture* embedded_texture, bool srgb)
{
	// try get
	const int texture_id = GetTextureIdStatic(filename);
	if (texture_id >= 0)
	{
		assert(texture_id < m_textures.size());
		return m_textures[texture_id];
	}
	// load
	auto texture_srv = LoadTextureFromMemory(m_device, embedded_texture, srgb);
	AddTextureStatic(filename, texture_srv);
	return texture_srv;
}
