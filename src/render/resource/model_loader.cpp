...
void  ModelLoader::AiModelLoader::LoadMaterial(const LoadingContext& loading_context, const aiMaterial* material, ModelTextureData& mesh_texture_data) const
{
	...
	// try emissive (blender)
	mesh_texture_data.m_emissive = LoadTextureOfType(loading_context, material, aiTextureType_EMISSIVE);
	if (!mesh_texture_data.m_diffuse.Get())
	{
		// try pbr(maya stingray pbs)
		mesh_texture_data.m_diffuse = LoadTextureOfType(loading_context, material, aiTextureType_EMISSION_COLOR);
	}
	// can be null
	...
	const LoadingContext& loading_context, const aiMaterial* material, enum aiTextureType type) const
{
	...
	if (material->GetTexture(type, 0, &texture_path) == AI_SUCCESS)
	{
		...
		bool srgb = type == aiTextureType_DIFFUSE || type == aiTextureType_BASE_COLOR
			|| type == aiTextureType_EMISSIVE || type == aiTextureType_EMISSION_COLOR;
		if (embeddedTex)
		{
			...
			if (embeddedTex->mHeight == 0)
			{
				...
				auto texture = texture_loader.GetOrLoadTextureFromMemory(texture_path.C_Str(), embeddedTex, srgb);
				//assert(texture.Get());
				return texture;
			}
			else
