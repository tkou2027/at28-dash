...
int TextureParticleManager::InitializeOrResetParticle(TextureParticleConfig& config, const XMFLOAT4X4& model_matrix)
{
	...
	if (model_id < 0)
	{
		...
		Geometry::CreatePlaneIndex(config.num_width, config.num_height, mesh_geo);
		ModelDesc model_desc{};
		model_loader.LoadMeshGeometryUnique(model_desc, mesh_geo);
		config.model_id = model_desc.model_id;
		// TODO: geometry id
	}
